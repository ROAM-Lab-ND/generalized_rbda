...
class DynamicsEngine {
 public:
   ...
   */
  size_t addCollisionBox(T mu, T rest, T depth, T width, T height,
                       const Vec3<T>& pos, const Mat3<T>& ori) {
    size_t collision_idx = _contact_constr->AddCollision(
  ...
  /*!
   * Remove a box
   * @param collision_idx : index of the collision
   * object to be removed from the the collision vector
   */
  void removeCollisionBox(size_t collision_idx) {
    _contact_constr->RemoveCollision(collision_idx);
  }
 ...
 private:
  ...
  SVec<T> bodyAcc,bodyAcc1,bodyAcc2;
  SVec<T> bodyVel,bodyVel1,bodyVel2;
