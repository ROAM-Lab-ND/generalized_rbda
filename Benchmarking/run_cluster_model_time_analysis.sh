...
if [[ $run_cpp -eq 1 ]]; then
    ./bin/cluster_model_timing_analysis
fi

# Run MATLAB and execute the script to plot the results
matlab -nodisplay -nosplash -nodesktop -r "try, addpath('../Benchmarking'); cluster_model_timing_analysis; catch ME, disp(ME.message), exit(1), end; exit(0);"
