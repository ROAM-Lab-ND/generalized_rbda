/* LCM type definition class file
 * This file was automatically generated by lcm-gen
 * DO NOT MODIFY BY HAND!!!!
 */

using System;
using System.Collections.Generic;
using System.IO;
using LCM.LCM;
 
namespace LCMTypes
{
    public sealed class humanoid_parameters_lcmt : LCM.LCM.LCMEncodable
    {
        public long control_mode;
        public bool cheater_mode;
        public bool display_heightmap;
        public float key_up;
        public float key_down;
        public float key_left;
        public float key_right;
        public float turn_right;
        public float stick_right_vertical;
        public float stick_right_horizontal;
        public float stick_left_vertical;
        public float stick_left_horizontal;
        public bool jump_trigger;
        public bool xbox_ctrl;
 
        public humanoid_parameters_lcmt()
        {
        }
 
        public static readonly ulong LCM_FINGERPRINT;
        public static readonly ulong LCM_FINGERPRINT_BASE = 0x764dd05493543db5L;
 
        static humanoid_parameters_lcmt()
        {
            LCM_FINGERPRINT = _hashRecursive(new List<String>());
        }
 
        public static ulong _hashRecursive(List<String> classes)
        {
            if (classes.Contains("LCMTypes.humanoid_parameters_lcmt"))
                return 0L;
 
            classes.Add("LCMTypes.humanoid_parameters_lcmt");
            ulong hash = LCM_FINGERPRINT_BASE
                ;
            classes.RemoveAt(classes.Count - 1);
            return (hash<<1) + ((hash>>63)&1);
        }
 
        public void Encode(LCMDataOutputStream outs)
        {
            outs.Write((long) LCM_FINGERPRINT);
            _encodeRecursive(outs);
        }
 
        public void _encodeRecursive(LCMDataOutputStream outs)
        {
            outs.Write(this.control_mode); 
 
            outs.Write(this.cheater_mode); 
 
            outs.Write(this.display_heightmap); 
 
            outs.Write(this.key_up); 
 
            outs.Write(this.key_down); 
 
            outs.Write(this.key_left); 
 
            outs.Write(this.key_right); 
 
            outs.Write(this.turn_right); 
 
            outs.Write(this.stick_right_vertical); 
 
            outs.Write(this.stick_right_horizontal); 
 
            outs.Write(this.stick_left_vertical); 
 
            outs.Write(this.stick_left_horizontal); 
 
            outs.Write(this.jump_trigger); 
 
            outs.Write(this.xbox_ctrl); 
 
        }
 
        public humanoid_parameters_lcmt(byte[] data) : this(new LCMDataInputStream(data))
        {
        }
 
        public humanoid_parameters_lcmt(LCMDataInputStream ins)
        {
            if ((ulong) ins.ReadInt64() != LCM_FINGERPRINT)
                throw new System.IO.IOException("LCM Decode error: bad fingerprint");
 
            _decodeRecursive(ins);
        }
 
        public static LCMTypes.humanoid_parameters_lcmt _decodeRecursiveFactory(LCMDataInputStream ins)
        {
            LCMTypes.humanoid_parameters_lcmt o = new LCMTypes.humanoid_parameters_lcmt();
            o._decodeRecursive(ins);
            return o;
        }
 
        public void _decodeRecursive(LCMDataInputStream ins)
        {
            this.control_mode = ins.ReadInt64();
 
            this.cheater_mode = ins.ReadBoolean();
 
            this.display_heightmap = ins.ReadBoolean();
 
            this.key_up = ins.ReadSingle();
 
            this.key_down = ins.ReadSingle();
 
            this.key_left = ins.ReadSingle();
 
            this.key_right = ins.ReadSingle();
 
            this.turn_right = ins.ReadSingle();
 
            this.stick_right_vertical = ins.ReadSingle();
 
            this.stick_right_horizontal = ins.ReadSingle();
 
            this.stick_left_vertical = ins.ReadSingle();
 
            this.stick_left_horizontal = ins.ReadSingle();
 
            this.jump_trigger = ins.ReadBoolean();
 
            this.xbox_ctrl = ins.ReadBoolean();
 
        }
 
        public LCMTypes.humanoid_parameters_lcmt Copy()
        {
            LCMTypes.humanoid_parameters_lcmt outobj = new LCMTypes.humanoid_parameters_lcmt();
            outobj.control_mode = this.control_mode;
 
            outobj.cheater_mode = this.cheater_mode;
 
            outobj.display_heightmap = this.display_heightmap;
 
            outobj.key_up = this.key_up;
 
            outobj.key_down = this.key_down;
 
            outobj.key_left = this.key_left;
 
            outobj.key_right = this.key_right;
 
            outobj.turn_right = this.turn_right;
 
            outobj.stick_right_vertical = this.stick_right_vertical;
 
            outobj.stick_right_horizontal = this.stick_right_horizontal;
 
            outobj.stick_left_vertical = this.stick_left_vertical;
 
            outobj.stick_left_horizontal = this.stick_left_horizontal;
 
            outobj.jump_trigger = this.jump_trigger;
 
            outobj.xbox_ctrl = this.xbox_ctrl;
 
            return outobj;
        }
    }
}

