...
class rpc_data_lcmt(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(rpc_data_lcmt._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != rpc_data_lcmt._get_packed_fingerprint():
            raise ValueError("Decode error")
        return rpc_data_lcmt._decode_one(buf)
    ...
    def _get_hash_recursive(parents):
        if rpc_data_lcmt in parents: return 0
    ...
    def _get_packed_fingerprint():
        if rpc_data_lcmt._packed_fingerprint is None:
            rpc_data_lcmt._packed_fingerprint = struct.pack(">Q", rpc_data_lcmt._get_hash_recursive([]))
        return rpc_data_lcmt._packed_fingerprint
