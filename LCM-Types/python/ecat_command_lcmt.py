...
class ecat_command_lcmt(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(ecat_command_lcmt._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != ecat_command_lcmt._get_packed_fingerprint():
            raise ValueError("Decode error")
        return ecat_command_lcmt._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = ecat_command_lcmt()
    ...
    def _get_hash_recursive(parents):
        if ecat_command_lcmt in parents: return 0
    ...
    def _get_packed_fingerprint():
        if ecat_command_lcmt._packed_fingerprint is None:
            ecat_command_lcmt._packed_fingerprint = struct.pack(">Q", ecat_command_lcmt._get_hash_recursive([]))
        return ecat_command_lcmt._packed_fingerprint
