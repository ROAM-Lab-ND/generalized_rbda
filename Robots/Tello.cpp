...
namespace grbda
{
	...
	for (size_t i(0); i < 2; i++)
	{
	    ...
	    // Hip clamp rotor
	    const SpatialTransform hip_clamp_rotor_Xtree = i == 0 ? SpatialTransform(R_left_hip_clamp_rotor,
										     p_left_hip_clamp_rotor)
								  : SpatialTransform(R_right_hip_clamp_rotor,
										     p_right_hip_clamp_rotor);
	    const std::string hip_clamp_rotor_name = side + "-hip-clamp-rotor";
	    const SpatialInertia<double> hip_clamp_rotor_spatial_inertia = SpatialInertia<double>{
		hip_clamp_rotor_mass, hip_clamp_rotor_CoM, hip_clamp_rotor_inertia};
	    auto hip_clamp_rotor = model.registerBody(hip_clamp_rotor_name, hip_clamp_rotor_spatial_inertia,
						      hip_clamp_rotor_parent_name, hip_clamp_rotor_Xtree);
	    ...
	    const SpatialInertia<double> hip_clamp_spatial_inertia = SpatialInertia<double>{hip_clamp_mass,
		hip_clamp_CoM, hip_clamp_inertia};
	    ...
	    const SpatialInertia<double> hip_rotor_1_spatial_inertia = SpatialInertia<double>{hip_rotor_1_mass,
		hip_rotor_1_CoM, hip_rotor_1_inertia};
	    ...
	    const SpatialInertia<double> hip_rotor_2_spatial_inertia = SpatialInertia<double>{hip_rotor_2_mass,
		hip_rotor_2_CoM, hip_rotor_2_inertia};
	    ...
	    const SpatialInertia<double> gimbal_spatial_inertia = SpatialInertia<double>{gimbal_mass,
		gimbal_CoM, gimbal_inertia};
	    auto gimbal = model.registerBody(gimbal_name, gimbal_spatial_inertia, gimbal_parent_name,
					     gimbal_Xtree);
	    ...
	    const SpatialInertia<double> thigh_spatial_inertia = SpatialInertia<double>{thigh_mass,
		thigh_CoM, thigh_inertia};
	    ...
	    const SpatialTransform knee_ankle_rotor_1_Xtree = i == 0 ?
		SpatialTransform(R_left_knee_ankle_rotor_1, p_left_knee_ankle_rotor_1) :
		SpatialTransform(R_right_knee_ankle_rotor_1, p_right_knee_ankle_rotor_1);
	    ...
	    const SpatialTransform knee_ankle_rotor_2_Xtree = i == 0 ?
		SpatialTransform(R_left_knee_ankle_rotor_2, p_left_knee_ankle_rotor_2) :
		SpatialTransform(R_right_knee_ankle_rotor_2, p_right_knee_ankle_rotor_2);
	    ...
	    // Shin
	    const SpatialTransform shin_Xtree = i == 0 ? SpatialTransform(R_left_shin,
									  p_left_shin)
						       : SpatialTransform(R_right_shin,
									  p_right_shin);
	    const std::string shin_name = side + "-shin";
	    const std::string shin_parent_name = side + "-thigh";
	    const SpatialInertia<double> shin_spatial_inertia = SpatialInertia<double>{shin_mass,
		shin_CoM, shin_inertia};
	    auto shin = model.registerBody(shin_name, shin_spatial_inertia, shin_parent_name, shin_Xtree);

	    // Foot
	    const SpatialTransform foot_Xtree = i == 0 ? SpatialTransform(R_left_foot,
									  p_left_foot)
						       : SpatialTransform(R_right_foot,
									  p_right_foot);
	    const std::string foot_name = side + "-foot";
	    const std::string foot_parent_name = side + "-shin";
	    const SpatialInertia<double> foot_spatial_inertia = SpatialInertia<double>{foot_mass,
		foot_CoM, foot_inertia};
	    ...
	    auto knee_ankle_differential_generalized_joint = 
		std::make_shared<GeneralizedJoints::TelloKneeAnkleDifferential>(
